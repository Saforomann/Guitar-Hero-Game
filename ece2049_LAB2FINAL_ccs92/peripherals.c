...
{
    // Calculate the timer count to match the frequency of the note
    unsigned int timerCount = (unsigned int)(32768 / frequency); // Assuming ACLK runs at 32768 Hz

    P3SEL |= BIT5; // Select peripheral output mode for P3.5
    P3DIR |= BIT5; // Set P3.5 as output

    TB0CTL = TBSSEL__ACLK | ID__1 | MC__UP; // ACLK, divider=1, Up mode
    TB0CTL &= ~TBIE; // Disable Timer B interrupts for safety

    // Set the PWM period in ACLK ticks
    TB0CCR0 = timerCount; // Set the period of the PWM signal
    TB0CCTL0 &= ~CCIE; // Disable CCR0 interrupts

    // Configure CC register 5, which is connected to our PWM pin TB0.5
    TB0CCTL5 = OUTMOD_7; // Set/reset mode for PWM
    TB0CCTL5 &= ~CCIE; // Disable capture/compare interrupts
    TB0CCR5 = timerCount / 2; // 50% duty cycle
}
